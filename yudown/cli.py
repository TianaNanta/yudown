...
from yudown.utils import VerifyLink, choise_dir, validate_choice, validate_location

app = typer.Typer(
    rich_markup_mode="rich",
    help="Download Youtube Media from this script and have a wonderful output😋",
)
...
def history(
    ...
    if len(media) == 0:
        ...
        for idx, media in enumerate(media, start=1):
            table.add_row(
                str(idx),
                f"[cyan]{media.filename}[/cyan]",
                f"[yellow]{media.extension}[/yellow]",
                f"[green]{media.resolution}[/green]",
                f"[red]{media.link}[/red]",
            )
        print(table)
...
@app.command("search")
def make_search(
    search_query: Optional[str] = typer.Argument(
        "", help="The word you are searching for", show_default=False
    ),
    suggestion: Optional[bool] = typer.Option(
        None, "--suggestion", "-s", help="Show search suggestion", show_default=False
    ),
):
    ...
    for idx, resultat in enumerate(resultat, start=1):
        table.add_row(
            str(idx),
            f"[cyan]{resultat.title}[/cyan]",
            f"[yellow]{resultat.watch_url}[/yellow]",
        )
    print(table)
    ...
    if suggestion:
        for i in s.completion_suggestions:
            print(f"{i}")
        print(
            f"[bold green]{len(s.completion_suggestions)}[/bold green] suggestions founded\n"
        )
...
def PlaylistDownload(
    link: Optional[str] = typer.Argument(None, show_default=False),
        ...
        for video in playlist.videos:
            ...
            for idx, video in enumerate(video, start=1):
                table.add_row(
                    ...
                    f"[yellow]{video.watch_url}[/yellow]",
                )
            print(table)
                ...
                case "audio":
                    ...
                    for stream in audiobj:
                        table.add_row(
                            str(i),
                            f"[cyan]{title}[/cyan]",
                            f"[yellow]{stream.audio_codec}[/yellow]",
                            f"[green]{stream.mime_type}[/green]",
                        )
                        i += 1
                    print(table)
                    ...
                    if 1 <= strm < i:
                        try:
                            ...
                            down = typer.confirm(
                                f"Confirm downloading {title} {extension_to_download} ?"
                            )
                            if not down:
                                raise typer.Abort()
                            # command for downloading the video
                            print(
                                f"You're now downloading the audio with extension {extension_to_download}..."
                            )
                            file = audio[strm - 1]
                            file.download(locate)
                        except:
                            print(
                                "❌[bold red]Some Error, download not completed ![/bold red]❌"
                            )
                            raise typer.Abort()
                        else:
                            print(
                            ...
                            media = Media(
                                filename=title,
                                extension="audio",
                                resolution=fileExtension[strm - 1],
                                link=url,
                            )
                            create(media)
                            raise typer.Exit()
                ...
                case "video":
                    ...
                    if res == 1:
                        try:
                            ...
                            print(
                                f"Downloading {title} [bold italic green]{high}[/bold italic green]"
                            )

                            dwn.get_highest_resolution().download(
                                locate, filename=title + " " + high
                            )
                        except:
                        ...
                        try:
                            ...
                            print(
                                f"Downloading {title} [bold italic red]{low}[/bold italic red]"
                            )

                            dwn.get_lowest_resolution().download(
                                locate, filename=title + " " + low
                            )
                        except:
                            print(f"Error while downloading the video !")
                            raise typer.Abort()
                        ...
                        videobj = yt.streams.order_by("resolution").filter(
                            progressive="True"
                        )
                        ...
                        for stream in videobj:
                            table.add_row(
                                str(i),
                                f"[cyan]{title}[/cyan]",
                                f"[yellow]{stream.resolution}[/yellow]",
                                f"[green]{stream.mime_type}[/green]",
                            )
                            i += 1
                        print(table)
                        ...
                        if 1 <= strm < i:
                            try:
                                ...
                                downl = typer.confirm(
                                    f"Confirm downloading {title} {resolution_to_download} ?"
                                )
                                if not downl:
                                    raise typer.Abort()
                                print(
                                    f"You're now downloading the video with resolution [bold italic green]{resolution_to_download}[/bold italic green]..."
                                )

                                videos[strm - 1].download(
                                    locate,
                                    filename=title + " " + resolution_to_download,
                                )
                                ...
                                video3 = Media(
                                    filename=title,
                                    extension="mp4",
                                    resolution=resolution_to_download,
                                    link=url,
                                )
                                create(video3)
                                raise typer.Exit()
